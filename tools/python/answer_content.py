...
def task1_blocks(timeline: dict) -> list[Block]:
    ...
    return [
        Text(
            content=(
                "本任务对Nyx官方128³重子气体密度场进行体渲染展示。数据共100个时间步，"
                "小端float32存储，体素线性索引为z→y→x（index=z+128y+128²x）。"
...
def task4_blocks(timeline: dict) -> list[Block]:
    ...
    return [
        Text(
            content=(
                "交互分析页（/app.html）集成log密度直方图（D3.js）与vtk.js体渲染。"
                "用户可在直方图上拖拽框选密度区间，或通过快捷按钮选取Top 1%"
                f"（ρ≥{s99['p99']:.4f}）与Bottom 1%（ρ≤{s99['p01']:.4f}）。"
